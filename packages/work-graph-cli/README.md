...
Safe reads and idempotency-keyed mutations retry HTTP 503 responses with
bounded exponential backoff. This includes structured database-capacity
responses and plain-text 503 responses generated by Cloudflare before the API
can run its error handler. Lease claims remain single-attempt operations because
their outcome can be uncertain if the platform terminates the request after the
database commits it.
