...
ideas: ["intelligent-document-processing", "ocr", "nlp"]
---
...
[`params.yaml`](https://github.com/Robbie-Palmer/hq/blob/main/ml-pipelines/recipe-parsing/params.yaml)
...
([`ml-pipelines/recipe-parsing/src/lib/openrouter.ts`](https://github.com/Robbie-Palmer/hq/blob/main/ml-pipelines/recipe-parsing/src/lib/openrouter.ts)).
...
2. **Model churn will continue.** New frontier and open models ship monthly.
...
5. **Cooklang is the canonical output.** Whatever model is in the loop, the result lands in
   ...
   Cooklang text inside `body` is plain text, validated post-hoc by `cooklang-rs`. Structured-output
   support across providers is required for the envelope, even though the
...
Configure provider-level routing per request. Fallbacks, sort-by-price, sort-by-throughput,
sort-by-latency, ignore-list, only-list, zero-data-retention enforcement, and quantisation
filtering all live in the request body instead of platform config. This means:
...
* **358 models across 60+ providers** (live count, May 2026), Anthropic, OpenAI, Google (AI
  ...
  *open-weight long tail* (Together, DeepInfra, Fireworks, Groq, Cerebras, Nebius, AkashML,
  ...
  \~15 providers. This breadth enables load balancing and price arbitrage.
...
* **Sort shortcuts in the model slug**. `:nitro` for highest throughput, `:floor` for lowest
  ...
  experimentation later wants it, but the integration does not depend on it.
...
* **Multimodal in one API**. Text, images, and PDFs (URL or base64). The photo-to-recipe stage
  ...
  Grok Voice) endpoints. This is relevant if cooking mode's voice navigation
...
The Stripe-style passthrough is an estimate based on Vercel's "may apply" language. The actual
...
* **Above 1M BYOK requests per month**: 5% of the equivalent OpenRouter pay-as-you-go cost on the
  surplus.
* **BYOK is configured per provider**. You can BYOK Anthropic while staying on
...
3. **Long tail stays pay-as-you-go.** Painful providers (AWS Bedrock SigV4, Vertex service
   accounts) stay on OpenRouter's relationship; the 5.8% buys us out of IAM. Open-weight
   experimentation providers (Together, DeepInfra, Fireworks) get the same story; no account for a
   one-week test.
...
The cross-provider LLM access market splits cleanly into two categories with different
operating models:
...
* **Cons**: Smaller catalogue (275 vs OpenRouter's 358 models, live counts May 2026) and
  ...
  deployment. We are on [Cloudflare Pages (ADR 011)](/projects/personal-knowledge-graph/adrs/011-cloudflare-pages)
...
> **Composable, not an alternative.** The [Vercel AI SDK](https://ai-sdk.dev/) (the
> client/server *library*, with `useChat`, `useCompletion`, `useObject` and streaming primitives) is a
...
* **Pros**: Sits inline with the rest of the Cloudflare estate ([ADR 011](/projects/personal-knowledge-graph/adrs/011-cloudflare-pages),
  [ADR 029](/projects/personal-knowledge-graph/adrs/029-cloudflare-images), [ADR 039](/projects/personal-knowledge-graph/adrs/039-cloudflare-r2)).
...
* **Cons**: To use Claude via Bedrock through AI Gateway, the project still needs an AWS account,
  ...
  if we wanted a single pane of glass across the whole product (Workers, Neon Postgres, AI, business
...
* **Decision**: **Rejected as the primary access layer.** Worth layering in front of OpenRouter
  *later* if user-facing AI features come to depend on guardrails, the one realistic
...
Vercel's 275. Portkey operates only as a router. Pricing is per-log (\~$49/mo for
...
* **Decision**: **Rejected.** Strong enterprise feature set we don't need, BYOK at the
  ...
  toward enterprise buyers and away from self-serve developers.
...
* **Third-party dependency on the live inference path.** Once user-facing AI features ship
  ...
  planning, nutritional analysis), product UX depends on OpenRouter availability. An
  OpenRouter outage degrades onboarding, blocks live transcription, breaks meal
...
* **We own per-user quotas, metering, and abuse protection.**
  ...
  per-user budget caps live in our Workers backend and PostgreSQL database on Neon. OpenRouter is
  the inference layer; the metering and access-control layer is ours to build. If our backend logic
  isn't enough,
...
* **Commercial single point of failure.** OpenRouter is a venture-backed startup with no
  ...
  Portkey), or to provider-direct requires a baseURL change and provider re-onboarding without a
  code rewrite. Keep the integration on the OpenAI-compatible API and treat
  vendor-specific OpenRouter features as optional.
...
* A feature comes to depend on prompt caching across providers (Anthropic-style cache control,
  OpenAI cached input pricing) and OpenRouter's pass-through coverage lags. At that point
