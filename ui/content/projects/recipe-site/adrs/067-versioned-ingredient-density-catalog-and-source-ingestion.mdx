...
Cloudflare Workflows orchestrates extract, snapshot, transform, stage, and validation steps. Each
upstream call and database batch is an independently retriable step with an idempotency key derived
from the source, release identifier, and checksum. Postgres stores the authoritative import status.
Workflow state is orchestration state and may expire after Cloudflare's retention period.
