...
It solved quick capture, hierarchy, and backlog ordering for a human-led
workflow. The semi-autonomous software development initiative now needs a
source of truth that workers can also coordinate through continuously.

The owner already reaches coding agents from mobile devices. Requiring a second
mobile or desktop interface would duplicate that access without strengthening
the system of record. The installed CLI must instead expose every planning
operation and enough structure for agents to explain the current plan.
...
Make the installed CLI the minimum complete operator interface. It must expose
capture, inspection, reprioritisation, and a deterministic projection of the
dependency chains blocking priority outcomes. The projection identifies ready
leaves and independent branches without estimating durations or delivery
dates. Agents may mediate these operations from any device. A graphical mobile
or desktop client remains optional and does not affect acceptance.
...
3. From an agent session away from the primary development machine, the owner
   can capture, inspect, and reprioritise work through the deployed CLI. The CLI
   can also project the highest-priority unresolved outcomes, their blocking
   dependency chains, ready leaves, independent parallel branches, and blocking
   attention requests in machine-readable and human-readable forms.
...
migrations, CLI compatibility, planning projections, and operational
maintenance become project responsibilities rather than vendor concerns.
Integrations need explicit directionality so that convenience views do not
...
Shortcut remains a strong human interface and already supports capture away
from a development machine. Expressing a complex dependency graph in it takes
too much manual maintenance. Its Kanban view does not expose the current
critical path or the safe parallel starting set. In practice, the owner must
repeatedly reread tickets, reconstruct those paths, and manually recompute
priorities before dispatching agents.
