...
session ID, client, provider and model used by each request, account class,
quota signal, metered cost, outcome, and handoff reason in private operational
telemetry. Expose only the useful work evidence and non-sensitive provenance
through Work Graph.
...
5. Private telemetry records the selected adapter, session ID, account class,
   per-request provider and model, quota signal, cost, outcome, and handoff.
   The public work record contains no credentials or sensitive quota details.
